        ...
        for i in range(num_tests):
            try:
                result = subprocess.run(
                    [sys.executable, '-c', func_test_list[i]],
                    capture_output=True,
                    text=True,
                    timeout=timeout,
                ...
                if result.returncode == 0:
                    ...
                    if not error_info:
                        error_info = result.stdout.strip()
                    failed_tests.append(f"{tests[i]} # output: {error_info}")
                    is_passing = False
            
            except subprocess.TimeoutExpired:
                failed_tests.append(f"{tests[i]} # output: TIMEOUT")
                is_passing = False
