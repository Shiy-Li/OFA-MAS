...
def base_args():
    ...
    # parser.add_argument('--data_dir', type=str, default='../OFA/FinetuneData_Generation/FinetuneData_OFA_Grammar',
    ...
    # Add model_name used by evaluation code
    parser.add_argument('--model_name', type=str, default='sentence-transformers/all-MiniLM-L6-v2',
                        help='Name of the sentence-transformer model to use.')

    # Using parse_known_args allows this to be a parent parser without causing errors
    args, _ = parser.parse_known_args()
    return parser, args


def pretrain_stage1_args():
    """Args for Stage 1: Unconditional Pre-training"""
...
def pretrain_stage2_args():
    """Args for Stage 2: Conditional Pre-training"""
    parent_parser, _ = base_args()
...
def finetune_stage3_args():
    # Get pretrain parser (as parent) and defaults/args
    pretrain_parser, defaults = base_args()

    # Create a fresh parser but reuse pretrain_parser as parent to inherit all pretrain args
    parser = argparse.ArgumentParser(description='OFA Fine-tuning Arguments',
                                     parents=[pretrain_parser],
                                     conflict_handler='resolve')

    # Now add/override arguments for fine-tuning
    ...
    # Override save_dir and other training parameters for fine-tuning
    parser.set_defaults(
...
def evaluate_args():
    # Re-initialize parser to avoid inheriting pretrain_args' parser
    parser = argparse.ArgumentParser(description='OFA Evaluation Arguments')

    # Get defaults from pretrain_args to maintain consistency
    _, defaults = base_args()

    # Evaluation settings
    parser.add_argument('--dataset', nargs='+', type=str,
                        default=['mmlu'],
                        # default=['humaneval','mmlu'],
                        # default=['multiarith', 'svamp'],
    ...
    # Use defaults from pretrain args where appropriate
    ...
    # Add other necessary args from pretrain_args for model initialization
...
# For backwards compatibility and to avoid breaking existing scripts immediately.
