...
def train(args, train_dataset, model, is_pretraining=False, unconditional=False):
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(dataloader):
            ...
            loss_graph, loss_vae, loss_balance, loss_gate_l1, accuracy = model(batch, beta_vae, unconditional=unconditional)
        ...
        avg_loss_vae = total_loss_vae / len(dataloader)
        ...
        print(f"  Losses -> Graph: {avg_loss_graph:.4f}, VAE: {avg_loss_vae:.4f}, Balance: {avg_loss_balance:.4f}, GateL1: {avg_loss_gate_l1:.4f}")
        ...
        if (epoch + 1) % 100 == 0:
            ...
            if unconditional:
                model_path = os.path.join(args.save_dir, f're_ofa_stage1_unconditional_{args.lr}_{epoch+1}_sparse.pth')
            elif is_pretraining:
                model_path = os.path.join(args.save_dir, f're_ofa_stage2_conditional_{args.lr}_{epoch+1}_sparse.pth')
            else:
                ...
                model_path = os.path.join(args.save_dir, f're_ofa_finetune_{dataset_str}_{args.lr}_{epoch+1}_sparse.pth')
