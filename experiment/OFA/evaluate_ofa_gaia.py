...
from GDesigner.prompt.gaia_prompt_set import ROLE_DESCRIPTION
...
# Helper functions from run_gaia.py
...
def get_dataset_config(dataset_name):
    """
    Loads the specified dataset and returns it along with a dictionary of
    dataset-specific processing functions and parameters.
    """
    ...
    if dataset_name == 'mmlu':
        ...
        # Ensure the split file exists, generated by the generation script
        ...
        dataset_specific_info = {
            # Use 'Question' key for task input, as seen in GAIA dataset structure.
            'record_to_input': lambda rec: {"task": rec["Question"]},
            # Extract the final answer from the model's output string.
            'postprocess_answer': lambda ans: ans.split("FINAL ANSWER: ")[-1] if "FINAL ANSWER: " in ans else ans,
            # Use the GAIA-specific scorer and 'Final answer' key for the ground truth.
...
async def evaluate(args, model, dataset, sentence_model, dataset_specific_info):
    """
    Main evaluation loop for the OFA model on a given dataset.
    """
    print(f"Starting evaluation on {args.dataset}...")
    
    # Unpack dataset-specific information
    ...
    for i_batch, record_batch in pbar:
        ...
        for record in record_batch:
            ...
            # Encode query and generate graph
...
async def main():
    ...
    try:
        ...
        # Load the rest of the weights, ignoring mismatches
    ...
    # --- Loop through datasets for evaluation ---
    all_dataset_logs = {}

    args.dataset = ['gaia_level_1', 'gaia_level_2', 'gaia_level_3'] # Evaluate all GAIA levels
    # args.dataset = ['gaia_level_3']  # Evaluate all GAIA levels
    for dataset_name in args.dataset:
        print("\n" + "#"*60)
        print(f"### EVALUATING ON: {dataset_name.upper()} ###")
        print("#"*60 + "\n")
        
        # Reset cost and token counters for each dataset run
        Cost.instance().reset()
        PromptTokens.instance().reset()
        CompletionTokens.instance().reset()
        
        # Create a copy of args to set the current dataset without affecting other runs
    ...
    # --- Run Evaluation ---
        ...
        # --- Log Summary for the current dataset ---
